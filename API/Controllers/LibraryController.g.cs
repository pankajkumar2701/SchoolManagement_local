...
{
    /// <summary>
    /// Controller responsible for managing library-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class LibraryController : ControllerBase
    {
        ...
        public LibraryController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new library to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Library.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Library>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Library.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Library.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Library updatedEntity)
        {
            ...
            this._context.Library.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Library> updatedEntity)
        {
            ...
            var existingEntity = this._context.Library.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Library.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
