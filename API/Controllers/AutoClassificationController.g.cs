...
{
    /// <summary>
    /// Controller responsible for managing autoclassification-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class AutoClassificationController : ControllerBase
    {
        ...
        public AutoClassificationController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new autoclassification to the database</summary>
        ...
        {
            _context.AutoClassification.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.AutoClassification.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<AutoClassification>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.AutoClassification.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.AutoClassification.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] AutoClassification updatedEntity)
        {
            ...
            this._context.AutoClassification.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<AutoClassification> updatedEntity)
        {
            ...
            var existingEntity = this._context.AutoClassification.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.AutoClassification.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
