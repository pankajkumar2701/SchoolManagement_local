...
{
    /// <summary>
    /// Controller responsible for managing classtype-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ClassTypeController : ControllerBase
    {
        ...
        public ClassTypeController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new classtype to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ClassType.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ClassType>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ClassType.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ClassType.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ClassType updatedEntity)
        {
            ...
            this._context.ClassType.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ClassType> updatedEntity)
        {
            ...
            var existingEntity = this._context.ClassType.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ClassType.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
