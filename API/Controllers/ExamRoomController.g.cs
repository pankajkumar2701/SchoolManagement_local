...
{
    /// <summary>
    /// Controller responsible for managing examroom-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ExamRoomController : ControllerBase
    {
        ...
        public ExamRoomController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new examroom to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ExamRoom.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ExamRoom>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ExamRoom.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ExamRoom.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ExamRoom updatedEntity)
        {
            ...
            this._context.ExamRoom.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ExamRoom> updatedEntity)
        {
            ...
            var existingEntity = this._context.ExamRoom.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ExamRoom.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
