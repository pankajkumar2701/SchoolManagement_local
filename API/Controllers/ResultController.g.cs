...
{
    /// <summary>
    /// Controller responsible for managing result-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ResultController : ControllerBase
    {
        ...
        public ResultController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new result to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Result.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Result>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Result.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Result.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Result updatedEntity)
        {
            ...
            this._context.Result.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Result> updatedEntity)
        {
            ...
            var existingEntity = this._context.Result.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Result.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
