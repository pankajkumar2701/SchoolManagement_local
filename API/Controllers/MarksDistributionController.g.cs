...
{
    /// <summary>
    /// Controller responsible for managing marksdistribution-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class MarksDistributionController : ControllerBase
    {
        ...
        public MarksDistributionController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new marksdistribution to the database</summary>
        ...
        {
            _context.MarksDistribution.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.MarksDistribution.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<MarksDistribution>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.MarksDistribution.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.MarksDistribution.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] MarksDistribution updatedEntity)
        {
            ...
            this._context.MarksDistribution.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<MarksDistribution> updatedEntity)
        {
            ...
            var existingEntity = this._context.MarksDistribution.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.MarksDistribution.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
