...
{
    /// <summary>
    /// Controller responsible for managing feeschedule-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class FeeScheduleController : ControllerBase
    {
        ...
        public FeeScheduleController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new feeschedule to the database</summary>
        ...
        {
            _context.FeeSchedule.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.FeeSchedule.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<FeeSchedule>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.FeeSchedule.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.FeeSchedule.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] FeeSchedule updatedEntity)
        {
            ...
            this._context.FeeSchedule.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<FeeSchedule> updatedEntity)
        {
            ...
            var existingEntity = this._context.FeeSchedule.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.FeeSchedule.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
