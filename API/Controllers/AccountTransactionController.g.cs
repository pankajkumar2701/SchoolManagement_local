...
{
    /// <summary>
    /// Controller responsible for managing accounttransaction-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class AccountTransactionController : ControllerBase
    {
        ...
        public AccountTransactionController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new accounttransaction to the database</summary>
        ...
        {
            _context.AccountTransaction.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.AccountTransaction.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<AccountTransaction>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.AccountTransaction.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.AccountTransaction.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] AccountTransaction updatedEntity)
        {
            ...
            this._context.AccountTransaction.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<AccountTransaction> updatedEntity)
        {
            ...
            var existingEntity = this._context.AccountTransaction.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.AccountTransaction.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
