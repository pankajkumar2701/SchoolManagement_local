...
{
    /// <summary>
    /// Controller responsible for managing students-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StudentsController : ControllerBase
    {
        ...
        public StudentsController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new students to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Students.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Students>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Students.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Students.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Students updatedEntity)
        {
            ...
            this._context.Students.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Students> updatedEntity)
        {
            ...
            var existingEntity = this._context.Students.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Students.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
