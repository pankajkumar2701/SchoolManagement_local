...
{
    /// <summary>
    /// Controller responsible for managing teacher-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class TeacherController : ControllerBase
    {
        ...
        public TeacherController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new teacher to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Teacher.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Teacher>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Teacher.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Teacher.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Teacher updatedEntity)
        {
            ...
            this._context.Teacher.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Teacher> updatedEntity)
        {
            ...
            var existingEntity = this._context.Teacher.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Teacher.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
