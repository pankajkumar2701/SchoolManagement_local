...
{
    /// <summary>
    /// Controller responsible for managing latefee-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class LateFeeController : ControllerBase
    {
        ...
        public LateFeeController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new latefee to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.LateFee.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<LateFee>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.LateFee.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.LateFee.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] LateFee updatedEntity)
        {
            ...
            this._context.LateFee.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<LateFee> updatedEntity)
        {
            ...
            var existingEntity = this._context.LateFee.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.LateFee.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
