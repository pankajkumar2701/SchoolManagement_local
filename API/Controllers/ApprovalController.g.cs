...
{
    /// <summary>
    /// Controller responsible for managing approval-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ApprovalController : ControllerBase
    {
        ...
        public ApprovalController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new approval to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Approval.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Approval>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Approval.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Approval.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Approval updatedEntity)
        {
            ...
            this._context.Approval.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Approval> updatedEntity)
        {
            ...
            var existingEntity = this._context.Approval.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Approval.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
