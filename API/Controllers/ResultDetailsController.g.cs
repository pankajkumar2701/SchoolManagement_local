...
{
    /// <summary>
    /// Controller responsible for managing resultdetails-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ResultDetailsController : ControllerBase
    {
        ...
        public ResultDetailsController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new resultdetails to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ResultDetails.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ResultDetails>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ResultDetails.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ResultDetails.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ResultDetails updatedEntity)
        {
            ...
            this._context.ResultDetails.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ResultDetails> updatedEntity)
        {
            ...
            var existingEntity = this._context.ResultDetails.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ResultDetails.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
