...
{
    /// <summary>
    /// Controller responsible for managing grade-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class GradeController : ControllerBase
    {
        ...
        public GradeController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new grade to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Grade.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Grade>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Grade.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Grade.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Grade updatedEntity)
        {
            ...
            this._context.Grade.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Grade> updatedEntity)
        {
            ...
            var existingEntity = this._context.Grade.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Grade.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
