...
{
    /// <summary>
    /// Controller responsible for managing shift-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ShiftController : ControllerBase
    {
        ...
        public ShiftController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new shift to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Shift.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Shift>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Shift.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Shift.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Shift updatedEntity)
        {
            ...
            this._context.Shift.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Shift> updatedEntity)
        {
            ...
            var existingEntity = this._context.Shift.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Shift.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
