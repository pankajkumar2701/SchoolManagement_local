...
{
    /// <summary>
    /// Controller responsible for managing resourcebooking-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ResourceBookingController : ControllerBase
    {
        ...
        public ResourceBookingController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new resourcebooking to the database</summary>
        ...
        {
            _context.ResourceBooking.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ResourceBooking.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ResourceBooking>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ResourceBooking.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ResourceBooking.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ResourceBooking updatedEntity)
        {
            ...
            this._context.ResourceBooking.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ResourceBooking> updatedEntity)
        {
            ...
            var existingEntity = this._context.ResourceBooking.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ResourceBooking.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
