...
{
    /// <summary>
    /// Controller responsible for managing workspace-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class WorkspaceController : ControllerBase
    {
        ...
        public WorkspaceController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new workspace to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Workspace.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Workspace>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Workspace.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Workspace.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Workspace updatedEntity)
        {
            ...
            this._context.Workspace.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Workspace> updatedEntity)
        {
            ...
            var existingEntity = this._context.Workspace.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Workspace.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
