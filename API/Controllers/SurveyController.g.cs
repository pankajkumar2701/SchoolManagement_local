...
{
    /// <summary>
    /// Controller responsible for managing survey-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class SurveyController : ControllerBase
    {
        ...
        public SurveyController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new survey to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Survey.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Survey>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Survey.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Survey.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Survey updatedEntity)
        {
            ...
            this._context.Survey.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Survey> updatedEntity)
        {
            ...
            var existingEntity = this._context.Survey.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Survey.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
