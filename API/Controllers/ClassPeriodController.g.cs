...
{
    /// <summary>
    /// Controller responsible for managing classperiod-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ClassPeriodController : ControllerBase
    {
        ...
        public ClassPeriodController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new classperiod to the database</summary>
        ...
        {
            _context.ClassPeriod.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ClassPeriod.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ClassPeriod>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ClassPeriod.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ClassPeriod.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ClassPeriod updatedEntity)
        {
            ...
            this._context.ClassPeriod.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ClassPeriod> updatedEntity)
        {
            ...
            var existingEntity = this._context.ClassPeriod.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ClassPeriod.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
