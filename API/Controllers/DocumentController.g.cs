...
{
    /// <summary>
    /// Controller responsible for managing document-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class DocumentController : ControllerBase
    {
        ...
        public DocumentController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new document to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Document.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Document>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Document.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Document.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Document updatedEntity)
        {
            ...
            this._context.Document.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Document> updatedEntity)
        {
            ...
            var existingEntity = this._context.Document.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Document.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
