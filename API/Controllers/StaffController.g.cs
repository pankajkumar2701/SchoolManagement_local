...
{
    /// <summary>
    /// Controller responsible for managing staff-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StaffController : ControllerBase
    {
        ...
        public StaffController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new staff to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Staff.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Staff>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Staff.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Staff.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Staff updatedEntity)
        {
            ...
            this._context.Staff.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Staff> updatedEntity)
        {
            ...
            var existingEntity = this._context.Staff.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Staff.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
