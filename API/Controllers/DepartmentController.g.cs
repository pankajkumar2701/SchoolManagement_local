...
{
    /// <summary>
    /// Controller responsible for managing department-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class DepartmentController : ControllerBase
    {
        ...
        public DepartmentController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new department to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Department.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Department>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Department.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Department.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Department updatedEntity)
        {
            ...
            this._context.Department.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Department> updatedEntity)
        {
            ...
            var existingEntity = this._context.Department.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Department.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
