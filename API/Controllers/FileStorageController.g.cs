...
{
    /// <summary>
    /// Controller responsible for managing filestorage-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class FileStorageController : ControllerBase
    {
        ...
        public FileStorageController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new filestorage to the database</summary>
        ...
        {
            _context.FileStorage.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.FileStorage.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<FileStorage>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.FileStorage.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.FileStorage.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] FileStorage updatedEntity)
        {
            ...
            this._context.FileStorage.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<FileStorage> updatedEntity)
        {
            ...
            var existingEntity = this._context.FileStorage.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.FileStorage.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
