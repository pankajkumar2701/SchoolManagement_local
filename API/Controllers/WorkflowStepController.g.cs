...
{
    /// <summary>
    /// Controller responsible for managing workflowstep-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class WorkflowStepController : ControllerBase
    {
        ...
        public WorkflowStepController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new workflowstep to the database</summary>
        ...
        {
            _context.WorkflowStep.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.WorkflowStep.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<WorkflowStep>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.WorkflowStep.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.WorkflowStep.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] WorkflowStep updatedEntity)
        {
            ...
            this._context.WorkflowStep.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<WorkflowStep> updatedEntity)
        {
            ...
            var existingEntity = this._context.WorkflowStep.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.WorkflowStep.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
