...
{
    /// <summary>
    /// Controller responsible for managing content-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ContentController : ControllerBase
    {
        ...
        public ContentController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new content to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Content.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Content>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Content.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Content.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Content updatedEntity)
        {
            ...
            this._context.Content.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Content> updatedEntity)
        {
            ...
            var existingEntity = this._context.Content.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Content.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
