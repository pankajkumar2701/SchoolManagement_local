...
{
    /// <summary>
    /// Controller responsible for managing resourcestatus-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ResourceStatusController : ControllerBase
    {
        ...
        public ResourceStatusController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new resourcestatus to the database</summary>
        ...
        {
            _context.ResourceStatus.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ResourceStatus.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ResourceStatus>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ResourceStatus.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ResourceStatus.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ResourceStatus updatedEntity)
        {
            ...
            this._context.ResourceStatus.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ResourceStatus> updatedEntity)
        {
            ...
            var existingEntity = this._context.ResourceStatus.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ResourceStatus.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
