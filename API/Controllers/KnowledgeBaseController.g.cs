...
{
    /// <summary>
    /// Controller responsible for managing knowledgebase-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class KnowledgeBaseController : ControllerBase
    {
        ...
        public KnowledgeBaseController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new knowledgebase to the database</summary>
        ...
        {
            _context.KnowledgeBase.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.KnowledgeBase.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<KnowledgeBase>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.KnowledgeBase.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.KnowledgeBase.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] KnowledgeBase updatedEntity)
        {
            ...
            this._context.KnowledgeBase.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<KnowledgeBase> updatedEntity)
        {
            ...
            var existingEntity = this._context.KnowledgeBase.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.KnowledgeBase.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
