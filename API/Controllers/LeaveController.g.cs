...
{
    /// <summary>
    /// Controller responsible for managing leave-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class LeaveController : ControllerBase
    {
        ...
        public LeaveController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new leave to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Leave.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Leave>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Leave.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Leave.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Leave updatedEntity)
        {
            ...
            this._context.Leave.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Leave> updatedEntity)
        {
            ...
            var existingEntity = this._context.Leave.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Leave.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
