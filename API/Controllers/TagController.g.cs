...
{
    /// <summary>
    /// Controller responsible for managing tag-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class TagController : ControllerBase
    {
        ...
        public TagController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new tag to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Tag.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Tag>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Tag.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Tag.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Tag updatedEntity)
        {
            ...
            this._context.Tag.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Tag> updatedEntity)
        {
            ...
            var existingEntity = this._context.Tag.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Tag.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
