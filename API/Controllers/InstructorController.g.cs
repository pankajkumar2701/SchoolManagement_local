...
{
    /// <summary>
    /// Controller responsible for managing instructor-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class InstructorController : ControllerBase
    {
        ...
        public InstructorController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new instructor to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Instructor.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Instructor>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Instructor.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Instructor.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Instructor updatedEntity)
        {
            ...
            this._context.Instructor.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Instructor> updatedEntity)
        {
            ...
            var existingEntity = this._context.Instructor.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Instructor.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
