...
{
    /// <summary>
    /// Controller responsible for managing workschedule-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class WorkScheduleController : ControllerBase
    {
        ...
        public WorkScheduleController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new workschedule to the database</summary>
        ...
        {
            _context.WorkSchedule.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.WorkSchedule.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<WorkSchedule>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.WorkSchedule.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.WorkSchedule.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] WorkSchedule updatedEntity)
        {
            ...
            this._context.WorkSchedule.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<WorkSchedule> updatedEntity)
        {
            ...
            var existingEntity = this._context.WorkSchedule.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.WorkSchedule.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
