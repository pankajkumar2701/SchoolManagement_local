...
{
    /// <summary>
    /// Controller responsible for managing room-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class RoomController : ControllerBase
    {
        ...
        public RoomController(SchoolManagement_localContext context)
        {
            _context = context;
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Room.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Room>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Room.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Room.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Room updatedEntity)
        {
            ...
            this._context.Room.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Room> updatedEntity)
        {
            ...
            var existingEntity = this._context.Room.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Room.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
