...
{
    /// <summary>
    /// Controller responsible for managing storageprovider-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StorageProviderController : ControllerBase
    {
        ...
        public StorageProviderController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new storageprovider to the database</summary>
        ...
        {
            _context.StorageProvider.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StorageProvider.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StorageProvider>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StorageProvider.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StorageProvider.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StorageProvider updatedEntity)
        {
            ...
            this._context.StorageProvider.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StorageProvider> updatedEntity)
        {
            ...
            var existingEntity = this._context.StorageProvider.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StorageProvider.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
