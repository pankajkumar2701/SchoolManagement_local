...
{
    /// <summary>
    /// Controller responsible for managing classroom-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ClassroomController : ControllerBase
    {
        ...
        public ClassroomController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new classroom to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Classroom.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Classroom>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Classroom.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Classroom.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Classroom updatedEntity)
        {
            ...
            this._context.Classroom.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Classroom> updatedEntity)
        {
            ...
            var existingEntity = this._context.Classroom.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Classroom.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
