...
{
    /// <summary>
    /// Controller responsible for managing semester-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class SemesterController : ControllerBase
    {
        ...
        public SemesterController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new semester to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Semester.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Semester>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Semester.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Semester.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Semester updatedEntity)
        {
            ...
            this._context.Semester.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Semester> updatedEntity)
        {
            ...
            var existingEntity = this._context.Semester.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Semester.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
