...
{
    /// <summary>
    /// Controller responsible for managing documentcategory-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class DocumentCategoryController : ControllerBase
    {
        ...
        public DocumentCategoryController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new documentcategory to the database</summary>
        ...
        {
            _context.DocumentCategory.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.DocumentCategory.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<DocumentCategory>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.DocumentCategory.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.DocumentCategory.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] DocumentCategory updatedEntity)
        {
            ...
            this._context.DocumentCategory.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<DocumentCategory> updatedEntity)
        {
            ...
            var existingEntity = this._context.DocumentCategory.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.DocumentCategory.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
