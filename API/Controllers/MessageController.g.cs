...
{
    /// <summary>
    /// Controller responsible for managing message-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class MessageController : ControllerBase
    {
        ...
        public MessageController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new message to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Message.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Message>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Message.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Message.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Message updatedEntity)
        {
            ...
            this._context.Message.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Message> updatedEntity)
        {
            ...
            var existingEntity = this._context.Message.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Message.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
