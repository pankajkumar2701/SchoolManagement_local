...
{
    /// <summary>
    /// Controller responsible for managing course-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class CourseController : ControllerBase
    {
        ...
        public CourseController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new course to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Course.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Course>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Course.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Course.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Course updatedEntity)
        {
            ...
            this._context.Course.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Course> updatedEntity)
        {
            ...
            var existingEntity = this._context.Course.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Course.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
