...
{
    /// <summary>
    /// Controller responsible for managing contactinformation-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ContactInformationController : ControllerBase
    {
        ...
        public ContactInformationController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new contactinformation to the database</summary>
        ...
        {
            _context.ContactInformation.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ContactInformation.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ContactInformation>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ContactInformation.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ContactInformation.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ContactInformation updatedEntity)
        {
            ...
            this._context.ContactInformation.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ContactInformation> updatedEntity)
        {
            ...
            var existingEntity = this._context.ContactInformation.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ContactInformation.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
