...
{
    /// <summary>
    /// Controller responsible for managing attendancepolicy-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class AttendancePolicyController : ControllerBase
    {
        ...
        public AttendancePolicyController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new attendancepolicy to the database</summary>
        ...
        {
            _context.AttendancePolicy.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.AttendancePolicy.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<AttendancePolicy>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.AttendancePolicy.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.AttendancePolicy.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] AttendancePolicy updatedEntity)
        {
            ...
            this._context.AttendancePolicy.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<AttendancePolicy> updatedEntity)
        {
            ...
            var existingEntity = this._context.AttendancePolicy.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.AttendancePolicy.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
