...
{
    /// <summary>
    /// Controller responsible for managing installment-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class InstallmentController : ControllerBase
    {
        ...
        public InstallmentController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new installment to the database</summary>
        ...
        {
            _context.Installment.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Installment.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Installment>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Installment.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Installment.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Installment updatedEntity)
        {
            ...
            this._context.Installment.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Installment> updatedEntity)
        {
            ...
            var existingEntity = this._context.Installment.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Installment.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
