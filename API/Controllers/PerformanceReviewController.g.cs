...
{
    /// <summary>
    /// Controller responsible for managing performancereview-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class PerformanceReviewController : ControllerBase
    {
        ...
        public PerformanceReviewController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new performancereview to the database</summary>
        ...
        {
            _context.PerformanceReview.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.PerformanceReview.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<PerformanceReview>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.PerformanceReview.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.PerformanceReview.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] PerformanceReview updatedEntity)
        {
            ...
            this._context.PerformanceReview.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PerformanceReview> updatedEntity)
        {
            ...
            var existingEntity = this._context.PerformanceReview.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.PerformanceReview.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
