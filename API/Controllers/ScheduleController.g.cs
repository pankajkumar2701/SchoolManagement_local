...
{
    /// <summary>
    /// Controller responsible for managing schedule-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ScheduleController : ControllerBase
    {
        ...
        public ScheduleController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new schedule to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Schedule.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Schedule>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Schedule.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Schedule.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Schedule updatedEntity)
        {
            ...
            this._context.Schedule.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Schedule> updatedEntity)
        {
            ...
            var existingEntity = this._context.Schedule.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Schedule.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
