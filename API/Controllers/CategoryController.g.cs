...
{
    /// <summary>
    /// Controller responsible for managing category-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class CategoryController : ControllerBase
    {
        ...
        public CategoryController(SchoolManagement_localContext context)
        {
            _context = context;
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Category.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Category>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Category.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Category updatedEntity)
        {
            ...
            this._context.Category.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Category> updatedEntity)
        {
            ...
            var existingEntity = this._context.Category.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Category.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
