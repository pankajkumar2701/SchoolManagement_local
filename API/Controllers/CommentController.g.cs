...
{
    /// <summary>
    /// Controller responsible for managing comment-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class CommentController : ControllerBase
    {
        ...
        public CommentController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new comment to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Comment.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Comment>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Comment.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Comment.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Comment updatedEntity)
        {
            ...
            this._context.Comment.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Comment> updatedEntity)
        {
            ...
            var existingEntity = this._context.Comment.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Comment.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
