...
{
    /// <summary>
    /// Controller responsible for managing campus-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class CampusController : ControllerBase
    {
        ...
        public CampusController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new campus to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Campus.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Campus>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Campus.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Campus.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Campus updatedEntity)
        {
            ...
            this._context.Campus.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Campus> updatedEntity)
        {
            ...
            var existingEntity = this._context.Campus.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Campus.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
