...
{
    /// <summary>
    /// Controller responsible for managing examschedule-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ExamScheduleController : ControllerBase
    {
        ...
        public ExamScheduleController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new examschedule to the database</summary>
        ...
        {
            _context.ExamSchedule.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ExamSchedule.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ExamSchedule>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ExamSchedule.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ExamSchedule.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ExamSchedule updatedEntity)
        {
            ...
            this._context.ExamSchedule.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ExamSchedule> updatedEntity)
        {
            ...
            var existingEntity = this._context.ExamSchedule.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ExamSchedule.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
