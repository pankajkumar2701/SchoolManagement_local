...
{
    /// <summary>
    /// Controller responsible for managing assignment-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class AssignmentController : ControllerBase
    {
        ...
        public AssignmentController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new assignment to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Assignment.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Assignment>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Assignment.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Assignment.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Assignment updatedEntity)
        {
            ...
            this._context.Assignment.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Assignment> updatedEntity)
        {
            ...
            var existingEntity = this._context.Assignment.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Assignment.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
