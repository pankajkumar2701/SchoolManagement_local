...
{
    /// <summary>
    /// Controller responsible for managing resourcetype-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ResourceTypeController : ControllerBase
    {
        ...
        public ResourceTypeController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new resourcetype to the database</summary>
        ...
        {
            _context.ResourceType.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ResourceType.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ResourceType>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ResourceType.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ResourceType.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ResourceType updatedEntity)
        {
            ...
            this._context.ResourceType.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ResourceType> updatedEntity)
        {
            ...
            var existingEntity = this._context.ResourceType.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ResourceType.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
