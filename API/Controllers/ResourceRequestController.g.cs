...
{
    /// <summary>
    /// Controller responsible for managing resourcerequest-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ResourceRequestController : ControllerBase
    {
        ...
        public ResourceRequestController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new resourcerequest to the database</summary>
        ...
        {
            _context.ResourceRequest.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ResourceRequest.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ResourceRequest>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ResourceRequest.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ResourceRequest.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ResourceRequest updatedEntity)
        {
            ...
            this._context.ResourceRequest.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ResourceRequest> updatedEntity)
        {
            ...
            var existingEntity = this._context.ResourceRequest.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ResourceRequest.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
