...
{
    /// <summary>
    /// Controller responsible for managing examresultnotification-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ExamResultNotificationController : ControllerBase
    {
        ...
        public ExamResultNotificationController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new examresultnotification to the database</summary>
        ...
        {
            _context.ExamResultNotification.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ExamResultNotification.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ExamResultNotification>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ExamResultNotification.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ExamResultNotification.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ExamResultNotification updatedEntity)
        {
            ...
            this._context.ExamResultNotification.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ExamResultNotification> updatedEntity)
        {
            ...
            var existingEntity = this._context.ExamResultNotification.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ExamResultNotification.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
