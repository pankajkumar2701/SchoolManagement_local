...
{
    /// <summary>
    /// Controller responsible for managing overtime-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class OvertimeController : ControllerBase
    {
        ...
        public OvertimeController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new overtime to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Overtime.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Overtime>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Overtime.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Overtime.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Overtime updatedEntity)
        {
            ...
            this._context.Overtime.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Overtime> updatedEntity)
        {
            ...
            var existingEntity = this._context.Overtime.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Overtime.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
