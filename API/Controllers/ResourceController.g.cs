...
{
    /// <summary>
    /// Controller responsible for managing resource-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ResourceController : ControllerBase
    {
        ...
        public ResourceController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new resource to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Resource.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Resource>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Resource.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Resource.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Resource updatedEntity)
        {
            ...
            this._context.Resource.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Resource> updatedEntity)
        {
            ...
            var existingEntity = this._context.Resource.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Resource.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
