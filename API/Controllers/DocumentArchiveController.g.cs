...
{
    /// <summary>
    /// Controller responsible for managing documentarchive-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class DocumentArchiveController : ControllerBase
    {
        ...
        public DocumentArchiveController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new documentarchive to the database</summary>
        ...
        {
            _context.DocumentArchive.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.DocumentArchive.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<DocumentArchive>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.DocumentArchive.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.DocumentArchive.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] DocumentArchive updatedEntity)
        {
            ...
            this._context.DocumentArchive.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<DocumentArchive> updatedEntity)
        {
            ...
            var existingEntity = this._context.DocumentArchive.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.DocumentArchive.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
