...
{
    /// <summary>
    /// Controller responsible for managing studentdocuments-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class StudentDocumentsController : ControllerBase
    {
        ...
        public StudentDocumentsController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new studentdocuments to the database</summary>
        ...
        {
            _context.StudentDocuments.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.StudentDocuments.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<StudentDocuments>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.StudentDocuments.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.StudentDocuments.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StudentDocuments updatedEntity)
        {
            ...
            this._context.StudentDocuments.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StudentDocuments> updatedEntity)
        {
            ...
            var existingEntity = this._context.StudentDocuments.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.StudentDocuments.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
