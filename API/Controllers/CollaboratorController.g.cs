...
{
    /// <summary>
    /// Controller responsible for managing collaborator-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class CollaboratorController : ControllerBase
    {
        ...
        public CollaboratorController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new collaborator to the database</summary>
        ...
        {
            _context.Collaborator.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Collaborator.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Collaborator>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Collaborator.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Collaborator.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Collaborator updatedEntity)
        {
            ...
            this._context.Collaborator.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Collaborator> updatedEntity)
        {
            ...
            var existingEntity = this._context.Collaborator.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Collaborator.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
