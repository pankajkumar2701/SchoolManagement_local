...
{
    /// <summary>
    /// Controller responsible for managing term-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class TermController : ControllerBase
    {
        ...
        public TermController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new term to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Term.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Term>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Term.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Term.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Term updatedEntity)
        {
            ...
            this._context.Term.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Term> updatedEntity)
        {
            ...
            var existingEntity = this._context.Term.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Term.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
