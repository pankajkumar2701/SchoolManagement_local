...
{
    /// <summary>
    /// Controller responsible for managing certificate-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class CertificateController : ControllerBase
    {
        ...
        public CertificateController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new certificate to the database</summary>
        ...
        {
            _context.Certificate.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Certificate.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Certificate>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Certificate.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Certificate.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Certificate updatedEntity)
        {
            ...
            this._context.Certificate.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Certificate> updatedEntity)
        {
            ...
            var existingEntity = this._context.Certificate.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Certificate.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
