...
{
    /// <summary>
    /// Controller responsible for managing paymentterms-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class PaymentTermsController : ControllerBase
    {
        ...
        public PaymentTermsController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new paymentterms to the database</summary>
        ...
        {
            _context.PaymentTerms.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.PaymentTerms.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<PaymentTerms>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.PaymentTerms.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.PaymentTerms.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] PaymentTerms updatedEntity)
        {
            ...
            this._context.PaymentTerms.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PaymentTerms> updatedEntity)
        {
            ...
            var existingEntity = this._context.PaymentTerms.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.PaymentTerms.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
