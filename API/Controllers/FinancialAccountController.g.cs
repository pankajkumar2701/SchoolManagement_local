...
{
    /// <summary>
    /// Controller responsible for managing financialaccount-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class FinancialAccountController : ControllerBase
    {
        ...
        public FinancialAccountController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new financialaccount to the database</summary>
        ...
        {
            _context.FinancialAccount.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.FinancialAccount.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<FinancialAccount>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.FinancialAccount.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.FinancialAccount.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] FinancialAccount updatedEntity)
        {
            ...
            this._context.FinancialAccount.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<FinancialAccount> updatedEntity)
        {
            ...
            var existingEntity = this._context.FinancialAccount.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.FinancialAccount.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
