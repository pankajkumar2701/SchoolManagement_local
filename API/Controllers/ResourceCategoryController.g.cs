...
{
    /// <summary>
    /// Controller responsible for managing resourcecategory-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ResourceCategoryController : ControllerBase
    {
        ...
        public ResourceCategoryController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new resourcecategory to the database</summary>
        ...
        {
            _context.ResourceCategory.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ResourceCategory.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ResourceCategory>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ResourceCategory.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ResourceCategory.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ResourceCategory updatedEntity)
        {
            ...
            this._context.ResourceCategory.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ResourceCategory> updatedEntity)
        {
            ...
            var existingEntity = this._context.ResourceCategory.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ResourceCategory.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
