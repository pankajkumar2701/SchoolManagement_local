...
{
    /// <summary>
    /// Controller responsible for managing feewaiver-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class FeeWaiverController : ControllerBase
    {
        ...
        public FeeWaiverController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new feewaiver to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.FeeWaiver.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<FeeWaiver>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.FeeWaiver.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.FeeWaiver.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] FeeWaiver updatedEntity)
        {
            ...
            this._context.FeeWaiver.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<FeeWaiver> updatedEntity)
        {
            ...
            var existingEntity = this._context.FeeWaiver.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.FeeWaiver.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
