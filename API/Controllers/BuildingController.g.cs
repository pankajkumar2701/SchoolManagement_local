...
{
    /// <summary>
    /// Controller responsible for managing building-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class BuildingController : ControllerBase
    {
        ...
        public BuildingController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new building to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Building.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Building>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Building.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Building.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Building updatedEntity)
        {
            ...
            this._context.Building.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Building> updatedEntity)
        {
            ...
            var existingEntity = this._context.Building.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Building.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
