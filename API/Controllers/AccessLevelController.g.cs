...
{
    /// <summary>
    /// Controller responsible for managing accesslevel-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class AccessLevelController : ControllerBase
    {
        ...
        public AccessLevelController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new accesslevel to the database</summary>
        ...
        {
            _context.AccessLevel.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.AccessLevel.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<AccessLevel>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.AccessLevel.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.AccessLevel.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] AccessLevel updatedEntity)
        {
            ...
            this._context.AccessLevel.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<AccessLevel> updatedEntity)
        {
            ...
            var existingEntity = this._context.AccessLevel.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.AccessLevel.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
