...
{
    /// <summary>
    /// Controller responsible for managing email-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class EmailController : ControllerBase
    {
        ...
        public EmailController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new email to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Email.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Email>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Email.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Email.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Email updatedEntity)
        {
            ...
            this._context.Email.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Email> updatedEntity)
        {
            ...
            var existingEntity = this._context.Email.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Email.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
