...
{
    /// <summary>
    /// Controller responsible for managing agenda-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class AgendaController : ControllerBase
    {
        ...
        public AgendaController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new agenda to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Agenda.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Agenda>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Agenda.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Agenda.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Agenda updatedEntity)
        {
            ...
            this._context.Agenda.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Agenda> updatedEntity)
        {
            ...
            var existingEntity = this._context.Agenda.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Agenda.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
