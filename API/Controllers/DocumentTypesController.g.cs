...
{
    /// <summary>
    /// Controller responsible for managing documenttypes-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class DocumentTypesController : ControllerBase
    {
        ...
        public DocumentTypesController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new documenttypes to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.DocumentTypes.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<DocumentTypes>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.DocumentTypes.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.DocumentTypes.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] DocumentTypes updatedEntity)
        {
            ...
            this._context.DocumentTypes.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<DocumentTypes> updatedEntity)
        {
            ...
            var existingEntity = this._context.DocumentTypes.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.DocumentTypes.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
