...
{
    /// <summary>
    /// Controller responsible for managing reportcard-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ReportCardController : ControllerBase
    {
        ...
        public ReportCardController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new reportcard to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.ReportCard.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<ReportCard>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.ReportCard.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.ReportCard.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] ReportCard updatedEntity)
        {
            ...
            this._context.ReportCard.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<ReportCard> updatedEntity)
        {
            ...
            var existingEntity = this._context.ReportCard.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.ReportCard.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
