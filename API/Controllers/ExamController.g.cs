...
{
    /// <summary>
    /// Controller responsible for managing exam-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ExamController : ControllerBase
    {
        ...
        public ExamController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new exam to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Exam.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Exam>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Exam.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Exam.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Exam updatedEntity)
        {
            ...
            this._context.Exam.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Exam> updatedEntity)
        {
            ...
            var existingEntity = this._context.Exam.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Exam.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
