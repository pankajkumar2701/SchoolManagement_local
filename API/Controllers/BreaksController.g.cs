...
{
    /// <summary>
    /// Controller responsible for managing breaks-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class BreaksController : ControllerBase
    {
        ...
        public BreaksController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new breaks to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Breaks.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Breaks>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Breaks.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Breaks.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Breaks updatedEntity)
        {
            ...
            this._context.Breaks.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Breaks> updatedEntity)
        {
            ...
            var existingEntity = this._context.Breaks.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Breaks.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
