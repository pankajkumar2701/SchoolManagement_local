...
{
    /// <summary>
    /// Controller responsible for managing workannouncement-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class WorkAnnouncementController : ControllerBase
    {
        ...
        public WorkAnnouncementController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new workannouncement to the database</summary>
        ...
        {
            _context.WorkAnnouncement.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.WorkAnnouncement.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<WorkAnnouncement>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.WorkAnnouncement.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.WorkAnnouncement.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] WorkAnnouncement updatedEntity)
        {
            ...
            this._context.WorkAnnouncement.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<WorkAnnouncement> updatedEntity)
        {
            ...
            var existingEntity = this._context.WorkAnnouncement.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.WorkAnnouncement.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
