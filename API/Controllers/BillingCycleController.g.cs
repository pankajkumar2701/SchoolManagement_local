...
{
    /// <summary>
    /// Controller responsible for managing billingcycle-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class BillingCycleController : ControllerBase
    {
        ...
        public BillingCycleController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new billingcycle to the database</summary>
        ...
        {
            _context.BillingCycle.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.BillingCycle.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<BillingCycle>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.BillingCycle.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.BillingCycle.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] BillingCycle updatedEntity)
        {
            ...
            this._context.BillingCycle.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<BillingCycle> updatedEntity)
        {
            ...
            var existingEntity = this._context.BillingCycle.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.BillingCycle.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
