...
{
    /// <summary>
    /// Controller responsible for managing timeoff-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class TimeOffController : ControllerBase
    {
        ...
        public TimeOffController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new timeoff to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.TimeOff.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<TimeOff>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.TimeOff.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.TimeOff.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] TimeOff updatedEntity)
        {
            ...
            this._context.TimeOff.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<TimeOff> updatedEntity)
        {
            ...
            var existingEntity = this._context.TimeOff.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.TimeOff.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
