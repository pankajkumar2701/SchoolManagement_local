...
{
    /// <summary>
    /// Controller responsible for managing question-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class QuestionController : ControllerBase
    {
        ...
        public QuestionController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new question to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Question.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Question>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Question.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Question.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Question updatedEntity)
        {
            ...
            this._context.Question.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Question> updatedEntity)
        {
            ...
            var existingEntity = this._context.Question.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Question.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
