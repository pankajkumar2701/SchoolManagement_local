...
{
    /// <summary>
    /// Controller responsible for managing school-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class SchoolController : ControllerBase
    {
        ...
        public SchoolController(SchoolManagement_localContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new school to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.School.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<School>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.School.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.School.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] School updatedEntity)
        {
            ...
            this._context.School.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<School> updatedEntity)
        {
            ...
            var existingEntity = this._context.School.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.School.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
